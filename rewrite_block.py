...
def main():
    ...
    for i, line in enumerate(lines):
        if "{/* ABOUT SECTION */}" in line:
            if start_idx == -1:
                start_idx = i - 1  # include the <div className="flex flex-col"> or similar
        if "{/* VIEW: THANK YOU PAGE */}" in line:
            end_idx = i
            break
...
"""
    ...
    # We replace from start_idx to end_idx - 1 (inclusive)
    new_lines = lines[:start_idx] + [new_middle + "\n" for new_middle in new_block.split('\n')] + lines[end_idx:]
